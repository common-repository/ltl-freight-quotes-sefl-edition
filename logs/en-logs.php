...
if (!class_exists('EnSeflLfqLogs')) {
    class EnSeflLfqLogs
    {
        ...
        {
            $obj_classs = new \SEFL_Curl_Request();
            $data = array(
                'serverName' => sefl_quotes_get_domain(),
                'licenseKey' => get_option('wc_settings_sefl_plugin_licence_key'),
                'lastLogs' => '25',
            ...
            $url = SEFL_FREIGHT_DOMAIN_HITTING_URL . '/request-log/index.php';
            $logs = $obj_classs->sefl_get_curl_response($url, $data);
            ...
            if (isset($logs['severity'], $logs['data']) && $logs['severity'] == 'SUCCESS') {
                ...
                foreach ($logs['data'] as $key => $shipment) {
                    ...
                    $class_name = 'sefl-log-' . $key . rand(1, 100);
